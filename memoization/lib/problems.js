...
/*THIS APPROACH IS A CORRECT ANSWER BUT DOES NOT GIVE MINIMUM AMOUNT OF COINS IN ALL COIN COMBOS*/
// function minChange(coins, amount, memo = {}) {

//     if (amount === 0) return 0;

//     if (coins[-1] > amount) {
//         coins += minChange(coins.slice(0, -1), amount);
//     }
//     else {
//         return parseInt(amount / coins.slice(-1)[0]) + minChange(coins.slice(0, -1), amount % coins.slice(-1)[0]);
//     }
// }
...
function minChange(coins, amount, memo = {0: 0}) {
    ...
    coins.forEach(coin => {
        if (coin <= amount)
            totalCoins.push(minChange(coins, amount - coin, memo) + 1);
    });

    return memo[amount] = Math.min(...totalCoins);
}
